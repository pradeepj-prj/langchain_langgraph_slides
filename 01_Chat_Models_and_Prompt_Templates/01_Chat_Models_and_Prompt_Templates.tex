...
\begin{frame}[fragile,shrink=5]{Temperature \& max\_tokens}
...
model_creative = ChatOpenAI(
  ...
  \begin{warningbox}[Token Budget]
    \texttt{max\_tokens} is output-only. Check: input + output < model limit.
  \end{warningbox}
\end{frame}
...
messages = template.format_messages(
  ...
  \begin{tipbox}[Key Pattern]
    \texttt{MessagesPlaceholder} inserts existing messages.
  \end{tipbox}
\end{frame}
...
\begin{frame}[fragile]{Few-Shot Chat: Final Prompt}
  \begin{codebox}[Composing with System]
    \begin{lstlisting}
final = ChatPromptTemplate.from_messages([
    ("system", "Find antonyms."),
    few_shot,
    ("user", "Word: {word}")
])
    \end{lstlisting}
  ...
  \begin{tipbox}[When to Use]
    Few-shot improves task-specific behavior. 3-5 examples often sufficient.
  \end{tipbox}
\end{frame}
...
\begin{frame}[fragile,shrink=8]{Composing Templates}
...
messages = system.messages + user_template.messages
  ...
  \end{codebox}
  \vspace{-0.1cm}
  \docref{Composition patterns}
\end{frame}

\begin{frame}[fragile]{Reusable Template Library}
  \begin{codebox}[Template Constants]
    \begin{lstlisting}
CODE_REVIEW_TEMPLATE = (
    ChatPromptTemplate.from_messages([
        ("system", "Review code. Be thorough."),
        ("user", "{code}")
    ]))
...
\begin{frame}[fragile]{Using Templates in Chains}
  \begin{codebox}[Composition Patterns]
    \begin{lstlisting}
# Use templates in chains
...
def create_template(role, task):
  ...
  \begin{tipbox}[Modularity]
    Store in separate module for consistency.
  \end{tipbox}
\end{frame}
...
\begin{frame}[fragile,shrink=5]{ainvoke() - Async Single Request}
...
asyncio.run(main())
  ...
  \vspace{-0.1cm}
  \begin{tipbox}[When to Use]
    Use async for web apps or concurrent processing.
  \end{tipbox}
\end{frame}
...
asyncio.run(main())
  ...
  \begin{tipbox}[Parallelism]
    Much faster than sequential \texttt{ainvoke()}.
  \end{tipbox}
\end{frame}
...
def call_with_retry(messages, max_retries=3):
    for attempt in range(max_retries):
        try:
            return ChatOpenAI().invoke(messages)
  ...
  \begin{tipbox}[Best Practice]
    Exponential backoff: don't hammer API.
  \end{tipbox}
\end{frame}
...
try:
    ...
    print("Timeout. Retry or use shorter prompt.")
    \end{lstlisting}
  \end{codebox}

  \docref{request\_timeout}
\end{frame}
...
# Batch / Stream
responses = model.batch([msg1, msg2])
...
\begin{frame}{Cheat Sheet: Parameters}
  \begin{footnotesize}
    ...
    \begin{tabular}{lp{5cm}}
      \toprule
      \textbf{Param} & \textbf{Use} \\
      \midrule
      temperature & 0 = task, 1.0 = balanced, 1.5+ = creative \\
      max\_tokens & output length limit \\
      top\_p & nucleus sampling (0-1) \\
      model & which LLM to use \\
      stop & halt generation strings \\
      \bottomrule
    \end{tabular}
  \end{footnotesize}

  \vspace{0.1cm}
  \begin{tipbox}[Defaults]
    Safe by default. Adjust only when needed.
  \end{tipbox}
\end{frame}
