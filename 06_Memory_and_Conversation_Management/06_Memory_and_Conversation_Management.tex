...
\begin{frame}
  ...
  \begin{codebox}[Message List]
    messages = [SystemMessage(content=``You are helpful''), HumanMessage(content=``My name is Alice''), AIMessage(content=``Nice to meet you!'')]
  \end{codebox}
\end{frame}
...
\begin{frame}
  ...
  \begin{codebox}[Usage]
    ...
    \hspace{2em} config=\{``configurable'': \{``session\_id'': ``user\_123''\}\} \\
    )
  \end{codebox}
