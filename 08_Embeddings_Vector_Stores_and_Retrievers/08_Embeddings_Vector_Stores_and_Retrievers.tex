...
\begin{frame}[fragile]{The Embeddings Interface: Core Methods}
...
\begin{frame}[fragile]{FAISS: In-Memory Vector Store for Prototyping}
  \begin{lstlisting}
...
documents = [
    "LangChain enables RAG applications",
    "Embeddings enable semantic search",
    "Vector stores index documents efficiently"
]
...
for doc in results:
  ...
  \vspace{0.2cm}
  \textbf{Pros:} Fast, in-memory, no setup. \quad \textbf{Cons:} Not persistent, single-machine.
\end{frame}
...
# Create/load persistent collection
vector_store = Chroma(
...
results = vector_store.similarity_search("query")
  ...
  \vspace{0.2cm}
  \textbf{Pros:} Persistent, local, metadata filtering. \quad \textbf{Cons:} Single-machine, limited scale.
\end{frame}
...
# Connect to Pinecone (requires API key)
...
# Add documents (synced to Pinecone servers)
vector_store.add_documents(documents)

# Search across distributed index
results = vector_store.similarity_search(
  ...
  \vspace{0.2cm}
  \textbf{Pros:} Fully managed, scales to billions, enterprise features. \quad \textbf{Cons:} Monthly cost, vendor lock-in.
\end{frame}
...
\begin{frame}{The Retriever Interface and How It Works}
...
\begin{frame}{Indexing Pipeline Visualization}
  \begin{tikzpicture}[
    node distance=1.5cm,
    every node/.style={draw, rounded corners, minimum width=2.5cm, minimum height=0.7cm, text centered, font=\small},
    input/.style={fill=lcblue!20, draw=lcblue},
    process/.style={fill=lcorange!20, draw=lcorange},
    storage/.style={fill=lcgreen!20, draw=lcgreen}
  ]
    % Top row: Indexing
    \node[input] (raw) {Raw Documents};
    \node[process, right=of raw] (split) {Text Splitter};
    \node[process, right=of split] (emb) {Embedding Model};
    \node[storage, right=of emb] (store) {Vector Store};
    ...
    % Storage details
    \node[below=1.2cm of store, text width=2cm, font=\tiny] (details) {
      \begin{tabular}{ll}
        Vector & Doc ID \\
        Meta & Source
      \end{tabular}
    };
    \draw[dashed, lcgreen] (store.south) -- (details.north);

    % Bottom row: Retrieval
    \node[input, below=2.5cm of raw] (query) {User Query};
    \node[process, right=of query] (qemb) {Embedding Model};
    \node[process, right=of qemb] (search) {ANN Search};
    \node[storage, right=of search] (results) {Top-K Docs};
  ...
  \vspace{0.3cm}
  \textbf{Key:} Documents and queries use the \emph{same} embedding model.
\end{frame}

% Frame 22: Retrieval Pipeline in Detail
\begin{frame}{Retrieval Pipeline in Detail}
  \begin{tikzpicture}[
    node distance=1.2cm,
    every node/.style={draw, rounded corners, minimum width=2.2cm, minimum height=0.65cm, text centered, font=\small},
    input/.style={fill=lcblue!20, draw=lcblue},
    process/.style={fill=lcorange!20, draw=lcorange},
    output/.style={fill=lcgreen!20, draw=lcgreen}
  ]
    % Query processing
    \node[input] (query) {Input Query};
    \node[process, below=of query] (preprocess) {Preprocess};
    \node[process, below=of preprocess] (embed) {Embed};

    % Search
    \node[process, below=of embed] (search) {ANN Search};

    % Post-processing
    \node[process, below=of search] (rerank) {Rerank/Filter};
    \node[output, below=of rerank] (final) {Final Results};

    % Edges
    \draw[->, thick] (query) -- (preprocess);
    \draw[->, thick] (preprocess) -- (embed);
    \draw[->, thick] (embed) -- (search);
    \draw[->, thick] (search) -- (rerank);
    \draw[->, thick] (rerank) -- (final);
  \end{tikzpicture}

  \vspace{0.3cm}
  \textbf{Stages:}
  \begin{enumerate}
    \item \textbf{Preprocess:} Normalize query (lowercase, strip whitespace)
    \item \textbf{Embed:} Convert to vector
    \item \textbf{Search:} Find nearest neighbors in vector store
    \item \textbf{Rerank:} Apply metadata filters, diversity, LLM scoring
  \end{enumerate}
\end{frame}
...
\begin{frame}[fragile]{Common Pitfall 2: Missing or Incomplete Metadata}
  \begin{warningbox}[Error Scenario]
...
\begin{frame}[fragile]{Real-World Example: RAG Chain with Retriever}
  \begin{lstlisting}[basicstyle=\ttfamily\tiny]
from langchain_chroma import Chroma
...
# Execute
result = chain.invoke({"input": "What is embeddings?"})
print(result["answer"])
  \end{lstlisting}
\end{frame}

% Frame 32: Real-World Example - Hybrid Search
\begin{frame}[fragile]{Real-World Example: Hybrid Search (Vector + Keyword)}
  \begin{lstlisting}[basicstyle=\ttfamily\tiny]
from langchain.retrievers import EnsembleRetriever
from langchain_chroma import Chroma
from langchain_community.retrievers import BM25Retriever
...
# Search uses both methods
results = ensemble.get_relevant_documents("ML model training")
# Semantically similar + keyword matches
  \end{lstlisting}

  \vspace{0.2cm}
  \textbf{Benefit:} Handles both semantic and keyword queries robustly.
\end{frame}
