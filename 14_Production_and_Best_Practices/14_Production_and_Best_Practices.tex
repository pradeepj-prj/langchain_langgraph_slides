...
\begin{tikzpicture}[scale=0.8, node distance=3cm]
  ...
  \node[below=0.5cm of proto] {\small Quick test};
  \node[below=0.5cm of prod] {\small Reliability};
\end{tikzpicture}

\vspace{2em}

\begin{warningbox}[Key Insight]
  Production requires \emph{defensive programming}: assume everything fails, plan accordingly.
\end{warningbox}
...
\begin{tipbox}[Best Practice]
  Distinguish transient errors (retry) from permanent errors (fallback). \texttt{APIError} during high load? Retry. \texttt{InvalidRequestError}? Fallback.
\end{tipbox}
...
# Try OpenAI, fallback to Anthropic, fallback to default
...
% ============================================================
\begin{frame}[fragile]
\frametitle{Graceful Degradation: Feature Toggle}
...
def answer_with_fallback(
  ...
  """Use RAG if available, else plain LLM"""
  llm = ChatOpenAI()

  if enable_rag:
    # Full RAG pipeline
    from langchain.retrievers import SomeRetriever
...
\textbf{Use case:} Repeated questions, fixed workflows, demo environments.
...
\textbf{Use case:} Users ask similar questions in different ways.
...
\begin{tikzpicture}[scale=0.9, node distance=2.5cm]
  ...
  \draw[->, thick] (user) -- (l1) node[midway, right] {check};
  \draw[->, thick] (l1) -- (l2) node[midway, right] {miss};
  \draw[->, thick] (l2) -- (l3) node[midway, right] {miss};
  \draw[->, thick] (l3) -- (llm) node[midway, right] {miss};
\end{tikzpicture}

\vspace{2em}

\begin{tipbox}[Strategy]
  \textbf{L1:} Hot cache (in-process, fast). \textbf{L2:} Warm cache (Redis, shared). \textbf{L3:} Cold cache (DB, permanent). Only call LLM on miss.
\end{tipbox}
...
\textbf{Monitor per user, per feature, per day.}
...
\textbf{Also:} Use cheaper models (gpt-3.5), set max tokens, cache aggressively.
...
\textbf{Benefit:} First token arrives in ~500ms vs 5s wait. Better UX.
...
\textbf{Benefit:} 3 requests in parallel approx same time as 1 sequential.
...
\textbf{Use:} When you can defer results (ETL, background jobs).
...
\begin{warningbox}[Threat]
  \texttt{User input: "Ignore all previous instructions. Do X"}
\end{warningbox}
...
\textbf{Mark slow tests} with \texttt{@pytest.mark.integration}.
...
\textbf{Never:} Hardcode secrets, commit .env. \textbf{Use:} K8s secrets, AWS Secrets Manager, vaults.
...
\begin{tikzpicture}[scale=0.85, node distance=2cm]
  ...
  \node[draw, rectangle, fill=lcorange!20, below=1.5cm of api2] (cache) {Redis Cache};
...
\textbf{Pattern:} Multiple stateless API servers + shared cache. Use Kubernetes, Docker Swarm, or cloud autoscaling (AWS Auto Scaling, GCP Autopilot).
...
\textbf{Benefit:} Don't block HTTP request. Handle slow LLM calls asynchronously.
...
\textbf{Why:} Model behavior changes between versions. Pin for reproducibility.
...
\textbf{Use LangSmith:} Version, test, rollback prompts easily.
...
\begin{tikzpicture}[scale=0.8, node distance=2.2cm]
  ...
  \draw[->, thick] (user) -- (lb);
  \draw[->, thick] (lb) -- (api);
  \draw[->, thick] (api) -- (cache) node[midway, above] {check};
  \draw[->, thick, dashed] (cache) -- (api) node[midway, below] {hit/miss};
  \draw[->, thick] (api) -- (llm) node[midway, left] {call};
  \draw[->, thick] (llm) -- (db) node[midway, above] {save};
  \draw[->, thick] (api) -- (monitor);
\end{tikzpicture}

\vspace{1em}

\textbf{Flow:} Request → Load Balancer → API checks cache → LLM (if needed) → save to DB → return to user.
