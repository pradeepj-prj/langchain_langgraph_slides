...
graph.add_node("node1", lambda x: {"messages": x["messages"] + ["step1"]})
graph.add_node("node2", lambda x: {"messages": x["messages"] + ["step2"]})
...
\begin{frame}
  ...
  \vspace{0.4cm}
  \begin{center}
    {\small \textcolor{lcblue}{\textbf{Quick reference—bookmark it!}}}
  \end{center}
\end{frame}
